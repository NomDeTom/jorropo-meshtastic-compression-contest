...
static int init_common(state_t *state, u8 *buf, size_t nbuf, real *cdf, size_t nsym)
{
	...
	printf("%s: number of symbols %zu too large for maximum %zu\n", __func__, nsym, (size_t)CDF_MAX_SYMB);
	return -1;
}
...
real *cdf_build(real *cdf, size_t *M, u8 *s, size_t N)
{
	...
	*M = maximum(s, N) + 1;
	...
	if ((M[0] + 1) > CDF_MAX_SYMB) {
		printf("%s: too many symbols (%zu > %zu)\n", __func__, M[0] + 1, (size_t)CDF_MAX_SYMB);
		return NULL;
	}
